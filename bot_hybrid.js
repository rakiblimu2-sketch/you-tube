...
// Helper: log admin actions to file for auditing
function logAdminAction(action, details = {}) {
    try {
        ...
        console.error('[ERROR] Failed to write admin log:', err);
    }
}
...
function loadAdminStats() {
    try {
        ...
        // Normalize totalUsers to numbers
        adminStats.totalUsers = new Set((data.totalUsers || []).map(u => (typeof u === 'string' ? parseInt(u) : u)));
        ...
        // Normalize bannedUsers to numbers to avoid string/number mismatches
        adminStats.bannedUsers = new Set((data.bannedUsers || []).map(u => (typeof u === 'string' ? parseInt(u) : u)));
...
bot.on('callback_query', async (query) => {
    ...
    if (data && data.startsWith('admin_')) {
        ...
        if (data === 'admin_stats') {
            ...
            await bot.answerCallbackQuery(query.id, { text: '📤 Preparing export...' });
            await sendAdminData(chatId);
        } else if (data === 'admin_download_logs') {
            await bot.answerCallbackQuery(query.id, { text: '📥 Preparing logs...' });
            await downloadBotLog(chatId);
            ...
            logAdminAction('maintenance_enabled', { admin: chatId });
            await showMaintenanceMenu(chatId, query.message.message_id);
            ...
            saveAdminStats(); // Save after changing maintenance mode
            logAdminAction('maintenance_disabled', { admin: chatId });
            await showMaintenanceMenu(chatId, query.message.message_id);
...
async function showAdminLogs(chatId, messageId) {
    // Try to read recent admin log file and show last lines
    const logFile = path.join(__dirname, 'bot.log');
    let logContent = null;
    try {
        if (fs.existsSync(logFile)) {
            const raw = fs.readFileSync(logFile, 'utf8');
            // Show only last ~200 lines to avoid big messages
            const lines = raw.trim().split(/\r?\n/);
            const tail = lines.slice(-200).join('\n');
            logContent = tail || 'No recent log entries.';
        }
    } catch (err) {
        console.error('[ERROR] Reading log file:', err);
    }

    const logsMessage = logContent ? `📝 *Bot Logs (last ${Math.min(200, logContent.split('\n').length)} lines)*\n\n` +
        '```' + logContent.substring(0, 4000) + '```' :
        `📝 Bot Logs\n\nNo log file found or an error occurred.`;

    const backKeyboard = {
        inline_keyboard: [
            [ { text: '📤 Export Data', callback_data: 'admin_export' }, { text: '📥 Download Logs', callback_data: 'admin_download_logs' } ],
            [ { text: '🔙 Back to Admin Panel', callback_data: 'admin_back' } ]
        ]
    };
    ...
    try {
        ...
        // In case message is too long or edit fails, send as plain text
        await bot.sendMessage(chatId, logsMessage.replace(/```/g, ''), { reply_markup: backKeyboard });
    }
}

// Send admin_data.json to admin as a downloadable file
async function sendAdminData(chatId) {
    try {
        const filePath = DATA_FILE;
        if (fs.existsSync(filePath)) {
            await bot.sendDocument(chatId, filePath, {}, { filename: 'admin_data.json' });
            logAdminAction('export_admin_data', { admin: chatId });
        } else {
            await bot.sendMessage(chatId, '❌ admin_data.json not found.');
        }
    } catch (err) {
        console.error('Error sending admin data:', err);
        await bot.sendMessage(chatId, `❌ Error exporting data: ${err.message}`);
    }
}

// Send the bot.log file to admin
async function downloadBotLog(chatId) {
    try {
        const logFile = path.join(__dirname, 'bot.log');
        if (fs.existsSync(logFile)) {
            await bot.sendDocument(chatId, logFile, {}, { filename: 'bot.log' });
            logAdminAction('download_logs', { admin: chatId });
        } else {
            await bot.sendMessage(chatId, '❌ Log file not found.');
        }
    } catch (err) {
        console.error('Error sending log file:', err);
        await bot.sendMessage(chatId, `❌ Error downloading logs: ${err.message}`);
    }
}
...
async function banUser(chatId, messageId, userId) {
    try {
        ...
        adminStats.bannedUsers.add(id);
        console.log(`[DEBUG] User ${id} banned by admin ${chatId}`);
        saveAdminStats(); // Save immediately after ban
        logAdminAction('ban_user', { admin: chatId, user: id });
        await showUserDetails(chatId, messageId, id);
    } catch (error) {
...
async function unbanUser(chatId, messageId, userId) {
    try {
        const id = (typeof userId === 'string') ? parseInt(userId) : userId;
        adminStats.bannedUsers.delete(id);
        console.log(`[DEBUG] User ${id} unbanned by admin ${chatId}`);
        saveAdminStats(); // Save immediately after unban
        logAdminAction('unban_user', { admin: chatId, user: id });
        await showUserDetails(chatId, messageId, id);
    } catch (error) {
...
async function clearStatistics(chatId, messageId) {
    ...
    logAdminAction('clear_statistics', { admin: chatId });
...
bot.on('message', async (msg) => {
    ...
    if (isAdmin(chatId) && adminStats.offerUpdateQueue && Array.isArray(adminStats.offerUpdateQueue) && adminStats.offerUpdateQueue.includes(chatId)) {
        ...
        if (newUrl.startsWith('http://') || newUrl.startsWith('https://')) {
            ...
            saveAdminStats();
            logAdminAction('update_offer', { admin: chatId, url: newUrl });
            // Use plain text to avoid Markdown parsing issues with URLs
...
// Start HTTP server to satisfy Render.com port requirement
...
server.listen(PORT, () => {
    console.log(`✅ HTTP server listening on port ${PORT} (for Render.com health checks)`);
});
